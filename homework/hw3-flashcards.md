...
We have provided several starter code files to help you write this flashcard app. You will need to understand and modify all of the JavaScript starter files that we give you to complete this assignment. This section provides more information about the starter code and how to get started with it.

We have also provided almost all of the CSS of this app. Unlike the JavaScript files, you should not have to modify `style.css`. The only CSS you should be adding is for animations, which you can add in `flashcard-style.css`.
...
- `constants.js`: This includes the `FLASHCARD_DECKS` global variable, which is an array of objects that represent a single flashcard deck. **We aren't expecting you to need to modify this file**, though you can if you'd like.

**Note:** We generally expect you to use and modify the provided classes to complete the assignment. However, if you feel strongly that you want to complete the assignment in a different way, e.g. you don't want to use the `MenuScreen` object for some reason, you are free to delete it or any of the other given starter code in favor of a class structure of your choosing.
- If you rely heavily on global variables, however, you will be marked down.
...
- `flashcard-style.css`: An empty CSS file for you to add any additional CSS rules you need to complete the assignment. You should only need to add CSS for animations.
...
<section class="part" markdown="1">
## 4. Flashcard screen and flashcard behavior
...
<video src="videos/hw3-animation.mp4" controls></video>
...
<video src="videos/hw3-click.mp4" controls></video>

- When you click a card, it should toggle between showing the word and the definition.
  - This is already implemented for you by toggling the `.show-word` class on the `.flashcard-box` `<div>` element.
...
<video src="videos/hw3-drag-right.mp4" controls></video>

**Drag left (incorrect answer)**
<video src="videos/hw3-drag-left.mp4" controls></video>

### Release flashcard
{:.no_toc}

**Release middle (no answer)**
<video src="videos/hw3-drag-middle.mp4" controls></video>

**Release left or right**
<video src="videos/hw3-drag-exit.mp4" controls></video>
...
- The `index.html` contains empty `span`s with classes `percent`, `correct`, and `incorrect`. Fill these in via JavaScript to get the desired output.
  - The stats are already styled for you in the provided CSS so long as you fill in the correct `span` elements with the data.
- The percentage should be rounded to the nearest whole number.

### Buttons
{:.no_toc}

There are two buttons displayed at the end of the flashcard session. The first button presented is different depending on whether the user got every flashcard correct.
